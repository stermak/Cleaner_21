...
then
	...
	if ! git clone --quiet https://github.com/stermak/Cleaner_21.git "$HOME"/"$tmp_dir" &>/dev/null;
	then
		sleep 0.5
	...
	sleep 1
	if [ "" == "$(diff "$HOME"/cleaner.sh "$HOME"/"$tmp_dir"/cleaner.sh)" ];
	then
		echo -e "\033[33m\n -- You already have the latest version of cclean --\n\033[0m"
		/bin/rm -rf "$HOME"/"${tmp_dir:?}"
		exit 0
	fi
	cp -f "$HOME"/"$tmp_dir"/cleaner.sh "$HOME" &>/dev/null
